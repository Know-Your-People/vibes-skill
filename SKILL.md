...
Files live in `vibes/`. Organised by type:
...
- User asks "what have I watched/listened to about X?" → search `vibes/` with expanded keywords
...
Use `grep` with expanded terms. Search type folders or all of `vibes/`.

```bash
# All jazz and soul albums
grep -ril "jazz\|blues\|soul\|r.b\|motown" vibes/albums/

# Finished shows with high rating
grep -rl "Rating: 5\|Rating: 4" vibes/shows/

# Podcasts about tech and startups
grep -ril "tech\|ai\|startup\|founders\|venture" vibes/podcasts/

# YouTube channels you follow
grep -rl "Status:.*following" vibes/youtube/

# YouTube channels with must-watch videos
grep -rl "## Must Watch" vibes/youtube/

# Vibes shared with specific person
grep -rl "\[\[marco" vibes/

# Want list across all types
grep -rl "Status:.*want" vibes/
...
- "3Blue1Brown is in your youtube/ folder but has no Must Watch list — anything from there worth saving?"
...
If it is not there yet, ask your human if they want to add **Vibes: check** to HEARTBEAT.md.
